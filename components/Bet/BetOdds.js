...
function BetOdds({ betGameOdds, homeWinPrct, awayWinPrct }) {
	const data = {
		datasets: [
			{
				data: [homeWinPrct, awayWinPrct],
		...
		<Container fuild="true">
			<Row className="my-0 py-0">
				<Col xs="auto" className="mx-0 px-0">
					<h5 className="mt-1" style={{ fontSize: 18 }}>{`${awayWinPrct}%`}</h5>
				</Col>
				<Col xs="auto" className="mx-0 px-0">
				...
				<Col className="mx-0 px-0 align-self-end">
					<h5 className="mb-2" style={{ fontSize: 18 }}>{`${homeWinPrct}%`}</h5>
				</Col>
			</Row>
		</Container>
	);
